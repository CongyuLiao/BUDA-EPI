function [img_msl]=recon_SMS_BUDA_CLv1_parfor (kspace_cor_ap, kspace_cor_pa,ky_idx_ap,ky_idx_pa,sens_gre_shift, AccY, AccZ,img_applied_topup,img_fieldmap,opt)
...
for ii=1:num_slc_raw
   ...
   parfor ii_slc=1:num_slc_raw 
        ...
        img_recon_applytopup=zeros(N(1),N(2),4);
        img_recon_applytopup(:,:,1:2) = cat(3, img_applied_topup(:,:,1,ii_slc), img_applied_topup(:,:,2,ii_slc+num_slc/AccZ));
        img_recon_applytopup(:,:,3:4) = cat(3, img_applied_topup(:,:,1,ii_slc), img_applied_topup(:,:,2,ii_slc+num_slc/AccZ));
...
%  img_msl=permute(reshape(permute(reshape(img_msl,[N(1),N(2),2,2,num_slc_raw]),[1,2,3,5,4]),[N(1),N(2),2,num_slc]),[1,2,4,3]);
